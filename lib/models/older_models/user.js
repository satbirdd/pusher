...
var User = nohm.model('User', {
  ...
  methods: {
    getEachGroupReadTime: function(callback) {
      Member.findAndLoad({ user_id: this.id }, function(err, members) {
        if (!err && members.length) {
          async.map(members, getInfo, callback);

          function getInfo(member, cb) {
            Group.load(member.p('group_id'), function(err, attrs) {
              if (err) {
                cb(null, null);
              } else {
                var info = {
                  id: member.p('group_id'),
                  time: member.p('read_time'),
                  name: this.p('name')
                }
                cb(null, info);
              }
